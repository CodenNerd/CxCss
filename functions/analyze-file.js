...
function extractClassNamesFromDynamicReactSyntax(fileContent, startingClassNames) {
        ...
        if (isClassNameStart) {
          ...
          if (["'", '"'].includes(currentDelimeter)) {
            if (char == ' ') {
              ...
              className && classNameSets[setId].push(className) && classNames.add(className)
              currentWord = ""
            } else if (char == delimeterPairs[currentDelimeter] && prevChar !== '\\') {
              const className = cleanUpClassName(currentWord)
                  className && classNameSets[setId].push(className) && classNames.add(className)
            ...
            if (char == '"' && prevChar !== '\\') {
              ...
              if (withinBackTicks || withinSingleQuotes || withinDoubleQuotes) {
                ...
                if ( char == delimeterPairs[currentDelimeter] && prevChar !== '\\' ) {
  ...
  function cleanUpClassName(className) {
    ...
    while (!cleanedClassName.match(startRegex) || !cleanedClassName.match(endRegex)) {
      if (!cleanedClassName.match(startRegex)) {
        cleanedClassName = cleanedClassName.substring(1).trim();
      }
      if (!cleanedClassName.match(endRegex)) {
        cleanedClassName = cleanedClassName.substring(0, cleanedClassName.length - 1).trim();
      }
